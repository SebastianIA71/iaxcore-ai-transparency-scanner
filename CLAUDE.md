...
The product cycle is **Scan → Fix → Rescan → Verify**, built in phases (Fase 0 → 7) each with an explicit exit gate defined in §10. As of this writing, Fase 0 (core contracts), Fase 1 (immutable platform: DB, queue, signing, rate-limited API), and the `runWorkerOnce` pipeline through Fase 2 (secure browser/SSRF) + Fase 3 (T1 detector) are wired together end-to-end and verified against the live Supabase database with real signing keys: claim → running → `runScan()` (real navigation, manifest) → `t1Detector.run()` (real channel/AI-evidence/disclosure detection) → `Finding` rows persisted via `createFindings` → signed `ReportArtifact` whose canonical JSON now includes those findings → `completed`, with the signature independently re-verifiable from what's in Postgres (see `apps/worker/src/index.integration.test.ts`, which needs `DATABASE_POOLED_URL` + `SIGNING_KEY_ID`/`SIGNING_PRIVATE_KEY_B64`/`SIGNING_PUBLIC_KEY_B64` and is skipped without them). What's still missing before either phase's exit gate is formally satisfied: Fase 2's SSRF battery hasn't been run against real hostile fixtures (only local-server tests), and Fase 3's provider-signature/phrase-pattern lists are a seed, not the curated, accuracy-measured catalog the gate calls for (§10-Fase 3: "precisión de `action_recommended` ≥95% en corpus etiquetado" — no labeled corpus exists yet). T2 detection (Fase 5) doesn't exist, `Evidence` (screenshots) is never captured — only `Finding` rows. Fase 4 (Fix/Rescan)'s pure-logic pieces exist in `packages/core` — `generateAiDisclosureFix()` (accessible ES/EN notice + HTML snippet + placement instructions, WCAG AA contrast verified by `contrastRatio()`) and `compareT1Assessments()` — and its exit gate is proven directly: a fixture goes from `action_recommended` to `aligned` after the generated notice is applied (`packages/detectors/src/t1Detector.test.ts`, last test). What Fase 4 doesn't have is a product surface — no Rescan button/API route, nothing links two `Evaluation`s of the same URL together; today a "rescan" is just calling `POST /api/scans` again with the same URL, which already works because nothing prevents it, but isn't surfaced as a feature. The web UI (`apps/web`) is still the scaffold placeholder — no scan form, no results page.
...
- `packages/detectors` — `t1Detector` implements `Detector<"t1">` for real: it opens its own SSRF-guarded Playwright page (via `@iaxcore/scanner`, independently of `runScan()` — `packages/core`'s `Detector` contract can't carry a `Page`, so each detector navigates on its own), finds and passively opens a chat/interaction widget (`channel.ts`), classifies AI-vs-human evidence and disclosure timing from the panel's visible text plus a seed provider-signature list (`signatures/index.ts`) and ES/EN/FR/DE/IT/PT phrase patterns (`patterns.ts`, `classification.ts`), and assembles the three §5.1 sub-findings plus the `t1.assessment` rollup via `deriveT1Assessment()`. The signature/pattern lists are intentionally a seed, not a finished catalog — see the B1 comment in `signatures/index.ts`. `runT1Detection(evaluationId, finalUrl, options)` is the testable inner function (`options.resolveHostname` lets tests point it at a local server, same pattern as `runScan`); `t1Detector.run()` is the thin `Detector`-shaped wrapper `apps/worker` calls. `t2Detector`/`t3Detector` are still typed stubs throwing "not implemented" (T2 is Fase 5, T3 is never-in-this-pilot).
- `packages/scanner` — the SSRF address classifier (`checkUrl`/`classifyAddress`), the Playwright browser wrapper with its request-interception SSRF guard (`installSsrfGuard`, byte/redirect/request limits), robots.txt parsing, deterministic page selection, consent-banner handling, a local `EvidenceStore`, and `runScan()` — the orchestrator that ties all of the above into one full scan and produces the per-evaluation manifest (`pages` with completed/excluded status and exclusion reason, `blocked_requests`, `consent_interaction`) that spec §10-Fase 2's exit gate requires.
...
- `apps/worker` — `src/index.ts` exports `runWorkerOnce(db, config)`: claim a job → mark running → `runScan()` the requested URL (`@iaxcore/scanner`) → `t1Detector.run()` against the scan's `finalUrl` (`@iaxcore/detectors`) → persist its findings via `createFindings` → sign a canonical report that now includes those findings → `completeEvaluation` with the scan's `finalUrl`/`pagesAnalyzed`/`manifest` → finish job. Independently unit-tested with `@iaxcore/db`/`@iaxcore/scanner`/`@iaxcore/detectors` all mocked, and separately verified against live Postgres (see `index.integration.test.ts`). `src/main.ts` is the actual polling-loop process entrypoint and isn't covered by tests. `evaluation.pagesRequested` (fixed at creation, §9's "hasta cinco páginas" cap) is passed to `runScan()` as `maxPages` — it's a cap set before the scan runs, not something the scan itself decides. The canonical report's `findings` array is built from `t1Detector`'s output mapped to `{detectorId, observationStatus, assessmentStatus, confidenceBand, summaryKey, detail}` — deliberately omitting `evaluationId` (redundant with the top-level one) and the DB-generated `id`/`createdAt`, so the signed payload is derivable from detector output alone, not a DB round-trip.
