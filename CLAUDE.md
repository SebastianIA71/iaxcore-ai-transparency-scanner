...
- `packages/scanner` — the SSRF address classifier (`checkUrl`/`classifyAddress`) lives here. The actual Playwright browser wrapper, page selection, and request interception described in spec §9/§10-Fase 2 don't exist yet.
...
- `apps/worker` — `src/index.ts` exports `runWorkerOnce(db, config)` (claim a job → mark running → sign a report → complete → finish job), independently testable; `src/main.ts` is the actual polling-loop process entrypoint and isn't covered by tests.
